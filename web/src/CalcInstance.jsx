...
export default function CalcInstance({ instanceKey = '', label, onRemove, theme = 'light', isComparison = false }) {
    ...
    useMemo(() => {
      ...
      if (isSplit) {
        const fixedRows = amortize({
          loanAmount: fixedAmt, annualRatePercent: inputs.fixedRatePercent,
          termYears: inputs.termYears, fixedRatePercent: inputs.fixedRatePercent,
          fixedPeriodYears: inputs.fixedPeriodYears, revertRatePercent: inputs.revertRatePercent,
          includeOffset: false,
        });
        const varRows = amortize({
          loanAmount: varAmt, annualRatePercent: inputs.splitVariableRatePercent,
          termYears: inputs.termYears, offsetStart: inputs.offsetStart,
          offsetMonthly: inputs.offsetMonthly, offsetLumps: inputs.offsetLumps,
          offsetWithdrawals: inputs.offsetWithdrawals, extraRecurring: inputs.extraRecurring,
          extraLumps: inputs.extraLumps, includeOffset: true,
        });
        ...
        withRows = Array.from({ length: maxLen }, (_, i) => {
          ...
          return {
            month: i + 1, payment: f.payment + v.payment,
            interest: f.interest + v.interest, principal: f.principal + v.principal,
            balance: f.balance + v.balance, offset: v.offset,
  ...
  return (
    <div className="calc-instance">
      {/* Scenario header shown in comparison mode */}
      {isComparison && (
      ...
      <div className="calc-body">
        ...
        <div className="panel">
          ...
          <div className="panel-section">
            ...
            <p className="offset-note">Offset applies to variable portion only — standard for AU lenders.</p>
            <div className="field">
